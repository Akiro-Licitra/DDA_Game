...
# For 2-5 seconds window, assuming 256 Hz sampling rate and analysis every BUFFER_SIZE samples
# We'll compute ratio per sample pulled from LSL
RATIO_WINDOW_SEC = 3  # e.g., 3-second window
RATIO_BUFFER_SIZE = RATIO_WINDOW_SEC * sampling_rate  # e.g., 3*256 = 768
ratio_window = deque(maxlen=RATIO_BUFFER_SIZE)

def analyze_data(data_combined, channel_names, start_time):
    """
    Compute Beta+Gamma / Alpha ratio, apply sliding window and normalization, and send via OSC.
    """
    dummy_data = np.array([data_combined[0], data_combined[0]])  # workaround for hypyp
    complex_signal = analyses.compute_freq_bands(dummy_data, sampling_rate, freq_bands)

    if complex_signal.shape[0] == 2: 
        subject_data = complex_signal[0]  # take first subject

        # Map bands to powers
        band_powers = {}
        for i, band_name in enumerate(freq_bands.keys()):
            if i < subject_data.shape[0]:
                band_powers[band_name] = get_power(subject_data[i])

        # Compute raw EEG ratio
        if "Beta" in band_powers and "Gamma" in band_powers and "Alpha" in band_powers:
            numerator = band_powers["Beta"] + band_powers["Gamma"]
            denominator = band_powers["Alpha"]
            raw_ratio = numerator / (denominator + 1e-6)  # avoid division by zero

            # Add to sliding window
            ratio_window.append(raw_ratio)

            # Normalize ratio to [0, 1] over current window
            min_ratio = min(ratio_window)
            max_ratio = max(ratio_window)
            normalized_ratio = (raw_ratio - min_ratio) / (max_ratio - min_ratio + 1e-6)

            # Send normalized value via OSC
            client.send_message("/eeg_ratio", float(normalized_ratio))
            print(f"Sent normalized EEG ratio: {normalized_ratio:.3f}")
